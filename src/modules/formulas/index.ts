...
export /*bundle */ class FormulaManager extends ReactiveModel<FormulaManager> {
    ...
    #initialize() {
        ...
        this.#instance = new objects[this.type](
            this,
            this.#plugin,
            this.#specs
        );
    ...
    getModels(variables: FormulaFields) {
        return variables.map((name) => {
            if (this.#plugin.formulas.has(name))
                return this.#plugin.formulas.get(name);
    ...
    getParser(data): ParserData {
        ...
        if (this.#parsers.has(data.formula))
            return this.#parsers.get(data.formula);
    ...
    async getParams(variables: string[]) {
        ...
        const build = async (value) => {
             ...
             */
            const element = formulas.has(value)
                ? formulas.get(value)
                : form.getField(value);
            if (!element)
                throw new Error(
                    `Field ${value} used in formula ${this.name}, not found in form ${form.name}, `
                );
            await element.isReady;
            params[value] = [undefined, '', null, NaN].includes(element.value)
                ? 0
                : element.value;
        };
