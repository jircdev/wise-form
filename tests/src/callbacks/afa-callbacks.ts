...
export const createAfaCallbacks = (formModel: FormModel) => {
    return {
        ...
        copyValue: (context: CallbackContext) => {
            const { form, field, to, from, propertyToCopy, valueSeparator } =
                context;
        ...
        getParametersTraffic: (context: CallbackContext) => {
            ...
            Object.entries(settings.fieldsToSetParameters).forEach(
                ([paramKey, fieldName]) => {
                    const param =
                        mockParameters[paramKey as keyof typeof mockParameters];
                    if (param) {
        ...
        selectItemJView: (context: CallbackContext) => {
            ...
            console.log('Selecting item in JView', {
                form,
                property,
                isSetInputs,
            });
        ...
        // Callback para añadir entries al titlesTable
        addEntryToTitlesTable: (context: CallbackContext) => {
            const { form } = context;
            const titlesTableField = form.getField('titlesTable');
            if (!titlesTableField) {
                console.error('titlesTable field not found');
                return;
            }

            // Obtener los campos del collapsible
            const tituloField = form.getField('titulo');
            const mesAnioField = form.getField('mesAnio');
            const pvpField = form.getField('pvp');
            const tiradaField = form.getField('tirada');
            const venta12MField = form.getField('venta12M');
            const porcentajeDescuentoField = form.getField(
                'porcentajeDescuento'
            );
            const porcentajeDerechoAutorField = form.getField(
                'porcentajeDerechoAutor'
            );
            const pvpDigitalField = form.getField('pvpDigital');
            const venta12MDigitalField = form.getField('venta12MDigital');
            const porcentajeDescuentoDigitalField = form.getField(
                'porcentajeDescuentoDigital'
            );
            const porcentajeDerechoAutorDigitalField = form.getField(
                'porcentajeDerechoAutorDigital'
            );

            // Obtener valores actuales de entries
            const currentEntries =
                titlesTableField.getProperties().entries || [];

            // Crear nuevo entry con los valores de los campos
            const newEntry = {
                titulo: tituloField?.value || '',
                mesAnio: mesAnioField?.value || '',
                pvp: pvpField?.value || 0,
                tirada: tiradaField?.value || 0,
                porcentajeVenta12M: venta12MField?.value
                    ? (venta12MField.value / (tiradaField?.value || 1)) * 100
                    : 0,
                porcentajeMargenBruto: 0, // Se calculará después
                inversionTotalTitulo: 0, // Se calculará después
                rentabilidad: 0, // Se calculará después
                rentabilidadAxPorciento: 0, // Se calculará después
            };

            // Añadir el nuevo entry a la lista
            const updatedEntries = [...currentEntries, newEntry];

            // Usar setValueField para actualizar los entries
            if (
                form.callbacks &&
                typeof form.callbacks.setValueField === 'function'
            ) {
                form.callbacks.setValueField({
                    form,
                    to: 'titlesTable',
                    property: 'entries',
        ...
        selectedItemAfa: (context: CallbackContext) => {
            ...
            if (callbacks && Array.isArray(callbacks)) {
                callbacks.forEach((callbackConfig: any) => {
                    const callback =
                        createAfaCallbacks(form)[callbackConfig.callback];
                    if (callback) {
        ...
        setPrevValues: (context: CallbackContext) => {
            ...
            fields.forEach((fieldConfig: any) => {
                ...
                if (fieldModel) {
                    ...
                    console.log('Saving previous value', {
                        field,
                        property,
                        currentValue,
                        isSaved,
